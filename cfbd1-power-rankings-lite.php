<?php
/*
Plugin Name: NCAA College Football Power Rankings Lite
...
function CFBD1SPRLPageURL() {
 $CFBD1SPRLpageURL = 'http';
 $CFBD1SPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $CFBD1SPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $CFBD1SPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $CFBD1SPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_cfbd1sprlstats() 
{
?>
<h2>NCAAF Power Rankings Lite</h2>
<?php cfbd1sprl_stats(); ?>
<?php
}

function cfbd1sprlstats_install()
{
register_sidebar_widget(__('NCAAF Power Rankings Lite'), 'widget_cfbd1sprlstats'); 
}
add_action("plugins_loaded", "cfbd1sprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'cfbd1sprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'cfbd1sprl_stats_remove' );

function cfbd1sprl_stats_install() 
{
...
add_option("cfbd1sprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("cfbd1sprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("cfbd1sprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NCCAF Power Rankings Installation", "Hi\n\nLITE NCAAF Power Rankings Activated at \n\n".CFBD1SPRLPageURL()."\n\nNCAAF Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function cfbd1sprl_stats_remove() 
{
/* Deletes the database field */
delete_option('cfbd1sprl_scroll_text_color');
delete_option('cfbd1sprl_scroll_text_color1');
delete_option('cfbd1sprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'cfbd1sprl_stats_admin_menu');

function cfbd1sprl_stats_admin_menu() {

add_options_page('NCAAF Power Rankings Lite', 'NCAAF Power Rankings Lite Settings', 'administrator', 'cfbd1-power-rankings-lite.php', 'cfbd1sprl_stats_plugin_page');
}
}
function cfbd1sprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('cfbd1sprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('cfbd1sprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('cfbd1sprl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/cfbd1_power_rankings_magpie_lite/cfbd1_power_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = cfbd1sprl_fetch_rss( $url );
...
function cfbd1sprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>NCAAF Power Rankings Lite Options Page</h2>
...
<!-- Start Advanced Plugins List -->
  ...
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NCAAF Power Rankings Scroller package') ?></h5>
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
    <label>
      <input type="button" name="DownloadUPgradeWidget" value="Download File" onclick="window.open('http://www.ibet.ws/download/cfbd1-power-rankings.zip', 'Download'); return false;" />
    </label>
    <br />
    <a href="http://www.ibet.ws/download/cfbd1-power-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
